...
router.post("/", upload.single("picture"), async (req, res) => {
  ...
  try {
    ...
    const msg =
      "Estas a unos pasos de poder disfrutar todos nuestros alojamientos Sólo da click al boton de abajo.";
...
router.get("/:idGuest/verify/:token", async (req, res) => {
  try {
    ...
    res.redirect(
      `https://nomade-khaki.vercel.app/${req.params.idGuest}/verify/${req.params.token}`
    );
  } catch (error) {
    res.status(404).send(error);
  }
