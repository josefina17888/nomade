  ...
  useEffect(() => {
    const getLodgingsInfo = async () => {
      try {
        ...
        let lodgingsVisibles = await lodgingsGot.filter(
          (e) => e.Visibility === true
        );
  ...
  const handleClick = (e) => {
    ...
    const getBookingsInfo = async () => {
      try {
        ...
        const unavailableDates = await bookingsGot.map((e) =>
    ...
    <div className={style.container1}>
      ...
      <div className={style.container3}>
        ...
        <div className={style.calendar}>
          ...
          {!bookings ? (
            <h5>Selecciona un alojamiento para conocer sus reservas</h5>
          ) : (
            bookings.map((e) => (
              <div>
                <div className={style.container1}>
                  <div className={style.container5}>
                    <div className={style.margin}>
                      <h6>
                        Check In: {new Date(e.checkIn).toLocaleDateString()}{" "}
                      </h6>
                    </div>
                    <div className={style.margin}>
                      <h6>
                        Check Out: {new Date(e.checkOut).toLocaleDateString()}{" "}
                      </h6>
                    </div>
                  </div>
                </div>
