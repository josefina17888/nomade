...
export default function Card({
  ...
  let arrCuantos = stateLodgings.map((e) =>
    e.lodgingId === id ? e.rating : false
  );
  let cuantos = 0;
  ...
  return (
    <div className={styles.card}>
      <div className={styles.img}>
        ...
        <div>
          <IoHeartOutline className={styles.fav} />
        </div>
        <div className={styles.icons}>
          <div className={styles.guests}>
            <VscPerson className={styles.guestsIcon} /> {guests}
          </div>
          <div className={styles.rating}>
            <IoIosStar className={styles.ratingIcon} />{" "}
            {promedio > 0 && promedio < 6
              ? promedio === 1.0 ||
                promedio === 2.0 ||
                promedio === 3.0 ||
                promedio === 4.0 ||
